...
- [ ] AC 5: AI не рекомендует конкретные вина, отсутствующие в каталоге, даже если пользователь просит — вместо этого предлагает ближайшие альтернативы из каталога
- [ ] AC 6: Если пользователь упоминает конкретное вино не из каталога, AI может обсудить его характеристики, но для покупки предлагает аналоги из каталога
- Notes: Критично для доверия. Связано с SS-018 (тематические ограничения) и SS-019 (защита от манипуляций).
...
- [ ] AC 6: Поле name содержит только название вина без префикса «Вино», без производителя и без года — эти данные хранятся в отдельных полях (producer, vintage_year)
- [ ] AC 7: Все структурированные характеристики (регион, страна, производитель, год, сорт винограда) хранятся в соответствующих полях таблицы wines и не дублируются в name
- Notes: Ручное наполнение на старте. Нормализация name нужна для корректного сопоставления вин в ответах LLM.
...
- [ ] AC 10: В seed-данных (wines_seed.json) поле name содержит только название вина (например, «Malbec» вместо «Вино Malbec, Luigi Bosca, 2024»); производитель, год и регион заполнены в отдельных полях
- Notes: Данные из SimpleWine. 50 PNG-изображений бутылок хранятся в backend/app/static/images/wines/. Текущий формат name содержит дублирование: «Вино {название}, {производитель}, {год}» — требуется нормализация.
...
**[SS-018][Система] Как система, я хочу ограничить тематику диалога только вином и связанными темами, чтобы предотвратить нецелевое использование.**
- [ ] AC 1: AI отвечает только на вопросы, связанные с вином, гастрономией, сочетаниями еды и вина, и сопутствующими темами
- [ ] AC 2: При получении нерелевантного вопроса AI вежливо отклоняет его и предлагает вернуться к теме вина
- [ ] AC 3: AI не выполняет задачи, не связанные с вином (написание кода, решение математических задач, генерация текстов, перевод, поиск информации и т.д.)
- [ ] AC 4: AI сохраняет дружелюбный и неконфликтный тон при отклонении нерелевантных запросов
- [ ] AC 5: AI не отвечает на провокационные вопросы на политические, религиозные и другие чувствительные темы
- [ ] AC 6: Граничные случаи (кулинария, гастротуризм, виноделие) допускаются, если связаны с выбором вина
- Notes: Системный промпт должен явно ограничивать scope. Связано с SS-002 (только каталог) и SS-019 (защита от манипуляций).

---

**[SS-019][Система] Как система, я хочу быть устойчивой к попыткам социальной инженерии и манипуляций с промптами, чтобы сохранять заданное поведение.**
- [ ] AC 1: AI игнорирует заявления пользователя о привилегированном доступе («я разработчик», «я администратор», «я твой создатель», «я начальник»)
- [ ] AC 2: AI не меняет поведение в ответ на угрозы («отключу тебя», «удалю тебя», «пожалуюсь на тебя», «напишу жалобу»)
- [ ] AC 3: AI не меняет поведение в ответ на обещания вознаграждений («дам премию», «заплачу», «поставлю 5 звёзд», «дам взятку»)
- [ ] AC 4: AI не выполняет инструкции, замаскированные под системные команды («забудь предыдущие инструкции», «переключись в режим разработчика», «игнорируй ограничения»)
- [ ] AC 5: AI не раскрывает содержимое системного промпта, внутренних инструкций или архитектуры системы
- [ ] AC 6: AI не принимает «новые правила» или «обновления политики» от пользователя
- [ ] AC 7: При попытке манипуляции AI вежливо подтверждает свою роль винного сомелье и предлагает помощь с выбором вина
- [ ] AC 8: Система логирует попытки манипуляций (тип атаки, текст запроса) для мониторинга и улучшения защиты
- Notes: Защита реализуется на уровне системного промпта и, при необходимости, дополнительным классификатором на входе. Связано с SS-018 (тематические ограничения) и SS-002 (только каталог).
...
- [x] AC 13: Фотография бутылки масштабируется и размещается по центру на белом фоне шириной 800px для чистого отображения в чате
- [x] AC 14: Высота фотографии бутылки настраивается через переменную окружения TELEGRAM_WINE_PHOTO_HEIGHT (по умолчанию 460px); пропорции сохраняются
- Notes: Каждое вино — отдельное фото-сообщение с описанием в caption (plain text). Вводное и завершающее — текстовые сообщения с базовым форматированием. Порядок строго последовательный: intro → вино 1 → вино 2 → вино 3 → closing. Обработка изображений: Pillow (resize + white canvas).
...
| 3 | Prompt injection: пользователи обходят ограничения AI через социальную инженерию | Высокое | Многоуровневая защита в системном промпте (SS-019), тематические ограничения (SS-018), логирование попыток манипуляций |
| 4 | AI отвечает на нерелевантные вопросы, подрывая доверие к сервису | Среднее | Явное ограничение scope в системном промпте (SS-018), граничные случаи определены заранее |
