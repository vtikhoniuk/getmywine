"""Reseed wines from updated catalog with local images

Revision ID: 012
Revises: 011
Create Date: 2026-02-10

Replaces all wines with data from the updated wines_seed.json
(sourced from catalog/wines.json) and sets image_url to local
static file paths.
"""
import json
import os
...
def upgrade() -> None:
    ...
    print("Deleted all existing wines.")
    ...
    print(f"Seeding {total} wines from catalog...")
    ...
    for i, wine in enumerate(data["wines"]):
        ...
        if api_key:
            ...
            print(f"  [{i+1}/{total}] {wine['name'][:50]}... ", end="", flush=True)
    ...
    print(f"Done! Seeded {total} wines from catalog.")


def downgrade() -> None:
    # Cannot restore old data; just clear
    op.execute("DELETE FROM wines")
