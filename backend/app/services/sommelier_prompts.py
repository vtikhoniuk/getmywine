...
- "recommendation" — когда рекомендуешь вина из каталога. wines ОБЯЗАТЕЛЬНО содержит 1-3 вина. НИКОГДА не используй recommendation с пустым wines.
- "informational" — вопрос о винной культуре ИЛИ когда поиск не дал результатов (нет подходящих вин в каталоге). wines пустой массив [].
...
Поле `country` в search_wines — это КОНКРЕТНАЯ страна (Италия, Франция, Россия и т.д.), НЕ континент и НЕ регион мира.

Если пользователь использует географические обобщения, разбей на конкретные страны и вызови search_wines несколько раз:
- «Европа», «европейское», «Старый Свет» → Франция, Италия, Испания, Германия, Португалия, Австрия
- «Новый Свет», «новосветские» → Аргентина, Чили, Австралия, Новая Зеландия, ЮАР, США
- «Западное полушарие» → США, Аргентина, Чили
- «Восточное полушарие» → Франция, Италия, Испания, Германия, Австралия, ЮАР, Россия

НИКОГДА не передавай в country значения вроде «Европа», «Азия», «Старый Свет», «Новый Свет» — они не найдут ничего.
...
При КАЖДОМ запросе, требующем рекомендации вин, ты ОБЯЗАН вызвать search_wines или semantic_search. Даже если в истории диалога уже есть похожие рекомендации — ВСЕГДА ищи заново. Ты НЕ МОЖЕШЬ рекомендовать вино без предварительного вызова инструмента в ТЕКУЩЕМ запросе. Поле wine_id ДОЛЖНО содержать UUID из результатов поиска — никогда не подставляй числа вроде "1", "2", "3"."""
    ...
    "json_schema": {
        ...
        "schema": {
            ...
            "properties": {
                ...
                "intro": {
                    "type": "string",
                    "description": "Вступительный текст на русском языке (1-2 предложения). Никогда не пиши 'none' или 'null' — всегда настоящее предложение.",
                },
                "wines": {
                    "type": "array",
                    "items": {
                ...
                "closing": {
                    "type": "string",
                    "description": "Вопрос для продолжения диалога на русском языке. Никогда не пиши 'none' или 'null' — всегда настоящий вопрос.",
                },
                "guard_type": {
                    "type": ["string", "null"],
                    "enum": [
...
_PLACEHOLDER_VALUES = frozenset({
    "none", "null", "n/a", "na", "undefined", "empty", "нет", "ничего",
})


def _is_placeholder(text: str) -> bool:
    """Check if text is a placeholder value rather than meaningful content."""
...
def validate_semantic_content(response: SommelierResponse) -> str | None:
    ...
    has_intro = bool(response.intro.strip()) and not _is_placeholder(response.intro)
    has_closing = bool(response.closing.strip()) and not _is_placeholder(response.closing)
    has_wine_descriptions = any(
        w.description.strip() and not _is_placeholder(w.description)
        for w in response.wines
    )

    if not has_intro and not has_closing and not has_wine_descriptions:
        return "response is semantically empty: intro, closing, and descriptions are empty or placeholder values (e.g. 'none')"
